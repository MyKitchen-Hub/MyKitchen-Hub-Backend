...
public class IngredientController {
        ...
        @Operation(summary = "Update ingredient", description = "Updates an existing ingredient (Authenticated users only). "
                        +
                        "All fields are optional. Name must be 1-100 characters, amount must be positive, unit must be 1-20 characters.")
        @ApiResponses(value = {
        ...
        @io.swagger.v3.oas.annotations.parameters.RequestBody(description = "Ingredient update data (all fields optional)", required = true, content = @Content(mediaType = "application/json", schema = @Schema(implementation = IngredientUpdateDto.class), examples = @io.swagger.v3.oas.annotations.media.ExampleObject(name = "Update ingredient", value = "{\"name\":\"Flour\",\"amount\":250.0,\"unit\":\"g\"}")))
        @PutMapping("/{id}")
